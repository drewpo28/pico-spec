    ...
    if (VIDEO::OSD && VIDEO::framecnt >= 10) {
        ...
        if ((VIDEO::OSD & 0x04) == 0 && !CPU::paused) {
            if (VIDEO::OSD == 1 && Tape::tapeStatus == TAPE_LOADING) {
                ...
                snprintf(OSD::stats_lin1, sizeof(OSD::stats_lin1), "CPU: %05d / IDL: %05d ", (int)(CPU::stFrame), (int)(ESPectrum::idle));
                ...
                snprintf(OSD::stats_lin3, sizeof(OSD::stats_lin3), "ST:%d TR:%d / SEC:%d ", ESPectrum::fdd.stepState, ESPectrum::fdd.track, ESPectrum::fdd.sector);
                snprintf(OSD::stats_lin4, sizeof(OSD::stats_lin4), "CPU:%6.2f Mhz", 3.5*(ESPectrum::multiplicator+1));
                OSD::drawStats();
            }
            totalseconds = 0;
